...
import { createSupplier, clearSupplierState, getSupplierById, updateSupplier } from "../redux/slice/supplier.slice";
...
export default function AddSupplier() {
  ...
  const { loading, error, success, supplierData } = useSelector((state) => state.supplier);
  ...
  useEffect(() => {
    if (isEditMode && supplierData && supplierData._id === id) {
      setFormData({
        ...
        role: supplierData.role || "supplyer",
      });
      // Only set imagePreview from server if no new image is selected
      if (!imageFile && supplierData.supplyer_image) {
        setImagePreview(`http://localhost:3000${supplierData.supplyer_image}`);
      } else if (!imageFile) {
        setImagePreview(null);
      }
    }
    // eslint-disable-next-line
  }, [supplierData, isEditMode, id]);
  ...
  const handleSubmit = (e) => {
    ...
    if (imageFile) {
      formData.append("supplyer_image", imageFile);
    }
    if (isEditMode) {
